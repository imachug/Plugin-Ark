...
def _bipToPrivatekey(bip):
    if len(bip.split()) != 12:
        raise CryptError("Private key must contain exactly 12 words")
    # Restore bit stream
    bits = ""
    for word in bip.split():
        try:
            ...
            raise CryptError("Private key must consist of a special wordlist")
    # Get private key
    ...
    if checksum != bits[128:]:
        raise CryptError("Privatekey checksum doesn't match")
...
def verify(data, valid_address, sign):  # Verify data using address and sign
    ...
    try:
        ...
        raise CryptError("Signature is invalid (coincurve)")
